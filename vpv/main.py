...
class Partical:
    ...
    def update(self, dt):
        ...
        if(abs(self.vx) > 0.2*sigma/dt):
            self.vx = random.random()-0.5
        if(abs(self.vy) > 0.2*sigma/dt):
            self.vy = random.random()-0.5
        if abs(self.ax) > 0.1*sigma/dt/dt:
            self.x+=0.03*(random.random()-0.5)
        if abs(self.ay) > 0.1*sigma/dt/dt:
            self.y+=0.03*(random.random()-0.5)
        self.x += self.vx*dt
        self.y += self.vy*dt
        self.ax = 0
...
log_file = open(f"vpv\log_2_{N}_{T_stab}.csv", "w")
log_file.write("t,T\n")
