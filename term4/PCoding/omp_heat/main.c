...
int main(int argc, char *argv[]){
    ...
    double maxarr[omp_get_max_threads()];
    #pragma parallel private(delta)
    {
    ...
    while(1){
        ...
        maxarr[omp_get_thread_num()] = 0;
        
        #pragma omp for
        for (i=1; i<N; i++){
            delta = fabs(unew[i]-u[i]);
            if(delta > maxarr[omp_get_thread_num()]) maxarr[omp_get_thread_num()] = delta;
        }
        ...
        #pragma omp single
        {
            ...
            for (i=0; i<omp_get_num_threads(); i++){
                if(maxarr[i] > maxdelta) maxdelta = maxarr[i];
            }
        }
        ...
        {
            count++;
            // printf("Step %d, maxdelta = %f, eps = %f\n", count, maxdelta, eps);
        }
        #pragma omp for
        for(i = 1; i < N; i++){
    	    u[i] = unew[i];
